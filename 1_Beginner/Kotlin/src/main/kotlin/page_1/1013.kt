import kotlin.math.abs

fun main() {
    /* [PT-BR]
     O comando readLine() aceita apenas 1 input em uma mesma linha, logo considerei a leitura como uma String,
     segmentei essa String a partir dos espaços vazios " " e usei o 'map' para converter os trechos de String em.
     Int. Finalmente, criei variáveis para receber os valores Int.

     [ENG]
     Command readLine() accepts only 1 input in the same line, therefore, I considered the input as a String.
     Then I split this String using blank spaces " " and used 'map' to convert the String segments to Int.
     Finally, I created variables to receive the Int values.
    */
    val read = readLine()!!.split(" ").map { it.toInt() }
    val A = read [0]
    val B = read [1]
    val C = read [2]
    var maior = (A+B+abs(A-B))/2
    if (maior == A) {
        maior = (A+C+abs(A-C))/2
    }else{
        maior = (B+C+abs(B-C))/2
    }
    println("$maior eh o maior")

}